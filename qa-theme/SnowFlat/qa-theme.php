...
class qa_html_theme extends qa_html_theme_base
{
	...
	{
        //TODO 变成WEB APP
        $this->output('<meta name="apple-touch-fullscreen" content="yes">');
        $this->output('<meta name="apple-mobile-web-app-capable" content="yes" />');
        $this->output('<meta name="viewport" content="width=device-width,minimum-scale=1.0,maximum-scale=1.0,user-scalable=no">');
        //TODO add web app icon
        $this->output('<link rel="apple-touch-icon-precomposed" href="./favicon.png">');
...
//		$this->content['css_src'][] = 'http://fonts.googleapis.com/css?family=Ubuntu:400,700,400italic,700italic';
	...
	public function title()
	{
        ...
        //TODO 增加编写按钮
        if(!isset($this->content['favorite'])){
            $this->output('<a href="./index.php?qa=ask" title="撰写"><i class="icon-comment qam-title-rss">发表</i></a>');
        }
	...
	{ // add view count to question list
        ...
        //TODO 隐藏投标按钮？
        $q_item['jackie_hide_vote_buttons'] = true;
		$this->voting($q_item);
		$this->a_count($q_item);
	...
	private function qam_user_account()
	{
		...
		if (qa_is_logged_in()) {
            ...
            $jackie_user_data = qa_db_select_with_pending(qa_db_user_profile_selectspec($userid, true));
            $handle = $jackie_user_data['name'];
